# -*- coding: utf-8 -*-
"""
BARE BONES Q-Learning Model - BASELINE
Parameters: alpha (fixed learning rate), beta (inverse temperature)
NO stimulation, NO lapse, NO kernel, NO decay, NO adaptive learning

This is the SIMPLEST possible Q-learning model to serve as a baseline.
Standard Q-learning: Q(t+1) = Q(t) + alpha * (reward - Q(t))
"""
...
def Calc_DistrQlearning_2Targs_Baseline(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	'''Bare bones Q-learning: fixed alpha, standard update rule'''
	...
	for i in range(len(chosen_target)-1):
		# STANDARD Q-LEARNING UPDATE (no modifications)
...
def loglikelihood_DistrQlearning_2Targs_Baseline(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
		Calc_DistrQlearning_2Targs_Baseline(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
	return log_prob_total


def DistrQlearning_2Targs_TwoStage_Baseline(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
	"""
	Bare bones baseline model - fit Block A and A' separately
	Only 2 parameters: alpha, beta
	This is standard Q-learning with no bells and whistles
	"""
	if num_trials_A>0 and num_trials_B>0:
		...
		print("\n" + "="*60)
		print("MODEL: Baseline (BARE BONES)")
		print("Standard Q-learning: Q(t+1) = Q(t) + α×(r - Q(t))")
		print("Parameters: alpha, beta ONLY")
		print("STAGE 1: Fitting Block A")
		print("="*60)
		
		Q_initial = 0.5 * np.ones(2)
		nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_Baseline(*args)
		x0 = [0.5, 3.0]  # [alpha, beta]
		
		result_A = op.minimize(nll, x0, 
							   args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
							   bounds=[(0,1), (0.1,50)],
							   method='Nelder-Mead')
		alpha_A, beta_A = result_A["x"]
		
		print(f"  alpha: {alpha_A:.4f}, beta: {beta_A:.4f}")
		
		Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
			Calc_DistrQlearning_2Targs_Baseline([alpha_A, beta_A], 
	...
	##Only fit using one block (for different task structure)##
		...
		alpha_Ap, beta_Ap = alpha_A, beta_A
	
	
	# BACKWARDS COMPATIBILITY: Return alpha twice for each block, set unused params to None
	return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
			alpha_A, alpha_A, None, None, None, beta_A,
			alpha_Ap, alpha_Ap, None, None, None, beta_Ap)
