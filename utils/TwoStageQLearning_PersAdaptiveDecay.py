# -*- coding: utf-8 -*-
"""
Perseverance + Decay Model - ADAPTIVE LEARNING, WITH DECAY, NO LAPSE
Parameters: alpha_base, decay, rho (perseverance), beta
...
def Calc_DistrQlearning_2Targs_PersAdaptiveDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	'''Adaptive Q-learning with perseverance and decay, no lapse'''
...
def loglikelihood_DistrQlearning_2Targs_PersAdaptiveDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
		Calc_DistrQlearning_2Targs_PersAdaptiveDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
	return log_prob_total


def DistrQlearning_2Targs_TwoStage_PersAdaptiveDecay(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
	...
	print("\n" + "="*60)
	print("MODEL: PersAdaptiveDecay (Adaptive + Decay, No Lapse)")
	print("α_effective = α_base × |prediction_error|")
	print("STAGE 1: Fitting Block A")
	print("="*60)
	
	Q_initial = 0.5 * np.ones(2)
	nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_PersAdaptiveDecay(*args)
	x0 = [0.5, 0.95, 0.0, 3.0]
	
	result_A = op.minimize(nll, x0, 
						   args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
						   bounds=[(0,1), (0.9,1), (-2,2), (0.1,50)],
						   method='Nelder-Mead')
	alpha_base_A, decay_A, rho_A, beta_A = result_A["x"]
	
	print(f"  alpha_base: {alpha_base_A:.4f}, decay: {decay_A:.4f}, rho: {rho_A:.4f}, beta: {beta_A:.4f}")
	
	Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
		Calc_DistrQlearning_2Targs_PersAdaptiveDecay([alpha_base_A, decay_A, rho_A, beta_A], 
	...
	x0_Ap = [alpha_base_A, decay_A, rho_A, beta_A]
	
	result_Ap = op.minimize(nll, x0_Ap,
							args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
							bounds=[(0,1), (0.9,1), (-2,2), (0.1,50)],
							method='Nelder-Mead')
	alpha_base_Ap, decay_Ap, rho_Ap, beta_Ap = result_Ap["x"]
	
	print(f"  alpha_base: {alpha_base_Ap:.4f}, decay: {decay_Ap:.4f}, rho: {rho_Ap:.4f}, beta: {beta_Ap:.4f}")
	print("="*60 + "\n")
	
	Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
		Calc_DistrQlearning_2Targs_PersAdaptiveDecay([alpha_base_Ap, decay_Ap, rho_Ap, beta_Ap], 
	...
	return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
			alpha_base_A, alpha_base_A, decay_A, rho_A, None, beta_A,
			alpha_base_Ap, alpha_base_Ap, decay_Ap, rho_Ap, None, beta_Ap)
