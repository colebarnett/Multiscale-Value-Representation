# -*- coding: utf-8 -*-
"""
Choice Kernel + Decay Model - DUAL-ALPHA, WITH DECAY, NO LAPSE
Parameters: pos_alpha, neg_alpha, decay, ck_weight, ck_decay, beta
...
def Calc_DistrQlearning_2Targs_KernelDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	'''Dual-alpha Q-learning with choice kernel and decay, no lapse'''
	pos_alpha = parameters[0]
	neg_alpha = parameters[1]
	decay = parameters[2]
	ck_weight = parameters[3]
	ck_decay = parameters[4]
	beta = parameters[5]
...
def loglikelihood_DistrQlearning_2Targs_KernelDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
		Calc_DistrQlearning_2Targs_KernelDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
	return log_prob_total


def DistrQlearning_2Targs_TwoStage_KernelDecay(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
	...
	print("\n" + "="*60)
	print("MODEL: KernelDecay (Dual-Alpha + Decay, No Lapse)")
	print("STAGE 1: Fitting Block A")
	print("="*60)
	
	Q_initial = 0.5 * np.ones(2)
	nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_KernelDecay(*args)
	x0 = [0.5, 0.5, 0.95, 0.0, 0.5, 3.0]
	
	result_A = op.minimize(nll, x0, 
						   args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
						   bounds=[(0,1), (0,1), (0.9,1), (-2,2), (0,1), (0.1,50)],
						   method='Nelder-Mead')
	pos_alpha_A, neg_alpha_A, decay_A, ck_weight_A, ck_decay_A, beta_A = result_A["x"]
	...
	Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
		Calc_DistrQlearning_2Targs_KernelDecay([pos_alpha_A, neg_alpha_A, decay_A, ck_weight_A, ck_decay_A, beta_A], 
	...
	x0_Ap = [pos_alpha_A, neg_alpha_A, decay_A, ck_weight_A, ck_decay_A, beta_A]
	
	result_Ap = op.minimize(nll, x0_Ap,
							args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
							bounds=[(0,1), (0,1), (0.9,1), (-2,2), (0,1), (0.1,50)],
							method='Nelder-Mead')
	pos_alpha_Ap, neg_alpha_Ap, decay_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap = result_Ap["x"]
	...
	Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
		Calc_DistrQlearning_2Targs_KernelDecay([pos_alpha_Ap, neg_alpha_Ap, decay_Ap, ck_weight_Ap, ck_decay_Ap, beta_Ap], 
	...
	return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
			pos_alpha_A, neg_alpha_A, decay_A, ck_weight_A, ck_decay_A, None, beta_A,
			pos_alpha_Ap, neg_alpha_Ap, decay_Ap, ck_weight_Ap, ck_decay_Ap, None, beta_Ap)
