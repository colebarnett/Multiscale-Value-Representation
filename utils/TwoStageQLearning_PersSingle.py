# -*- coding: utf-8 -*-
"""
Perseverance Model - SINGLE-ALPHA, NO DECAY, NO LAPSE
Parameters: alpha, rho (perseverance), beta
...
def Calc_DistrQlearning_2Targs_PersSingle(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	'''Single-alpha Q-learning with perseverance, no decay, no lapse'''
	alpha = parameters[0]
	rho = parameters[1]
	beta = parameters[2]
...
def loglikelihood_DistrQlearning_2Targs_PersSingle(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
		Calc_DistrQlearning_2Targs_PersSingle(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
	return log_prob_total


def DistrQlearning_2Targs_TwoStage_PersSingle(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
	...
	print("\n" + "="*60)
	print("MODEL: PersSingle (Single-Alpha, No Decay, No Lapse)")
	print("STAGE 1: Fitting Block A")
	print("="*60)
	
	Q_initial = 0.5 * np.ones(2)
	nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_PersSingle(*args)
	x0 = [0.5, 0.0, 3.0]
	...
	result_A = op.minimize(nll, x0, 
						   ...
						   method='Nelder-Mead')
	alpha_A, rho_A, beta_A = result_A["x"]
	
	print(f"  alpha: {alpha_A:.4f}, rho: {rho_A:.4f}, beta: {beta_A:.4f}")
	
	Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
		Calc_DistrQlearning_2Targs_PersSingle([alpha_A, rho_A, beta_A], 
	...
	x0_Ap = [alpha_A, rho_A, beta_A]
	
	result_Ap = op.minimize(nll, x0_Ap,
							args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
							bounds=[(0,1), (-2,2), (0.1,50)],
							method='Nelder-Mead')
	alpha_Ap, rho_Ap, beta_Ap = result_Ap["x"]
	
	print(f"  alpha: {alpha_Ap:.4f}, rho: {rho_Ap:.4f}, beta: {beta_Ap:.4f}")
	print("="*60 + "\n")
	
	Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
		Calc_DistrQlearning_2Targs_PersSingle([alpha_Ap, rho_Ap, beta_Ap], 
	...
	return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
			alpha_A, alpha_A, None, rho_A, None, beta_A,
			alpha_Ap, alpha_Ap, None, rho_Ap, None, beta_Ap)
