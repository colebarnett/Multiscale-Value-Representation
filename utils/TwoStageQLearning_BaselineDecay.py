# -*- coding: utf-8 -*-
"""
Q-Learning Model - BASELINE + DECAY
Parameters: alpha (fixed learning rate), decay (forgetting), beta (inverse temperature)
NO stimulation, NO lapse, NO kernel, NO adaptive learning

Standard Q-learning with decay/forgetting toward neutral value.
"""
...
def Calc_DistrQlearning_2Targs_BaselineDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	'''Standard Q-learning with decay/forgetting'''
	alpha = parameters[0]  # Fixed learning rate
...
def loglikelihood_DistrQlearning_2Targs_BaselineDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
		Calc_DistrQlearning_2Targs_BaselineDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
	return log_prob_total


def DistrQlearning_2Targs_TwoStage_BaselineDecay(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
	"""
	Baseline + Decay model - fit Block A and A' separately
	Only 3 parameters: alpha, decay, beta
	Standard Q-learning with forgetting
	"""
	...
	print("\n" + "="*60)
	print("MODEL: Baseline + Decay")
	print("Standard Q-learning with forgetting")
	print("Parameters: alpha, decay, beta ONLY")
	print("STAGE 1: Fitting Block A")
	print("="*60)
	
	Q_initial = 0.5 * np.ones(2)
	nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_BaselineDecay(*args)
	x0 = [0.5, 0.95, 3.0]  # [alpha, decay, beta]
	
	result_A = op.minimize(nll, x0, 
						   args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
						   bounds=[(0,1), (0.9,1), (0.1,50)],
						   method='Nelder-Mead')
	alpha_A, decay_A, beta_A = result_A["x"]
	
	print(f"  alpha: {alpha_A:.4f}, decay: {decay_A:.4f}, beta: {beta_A:.4f}")
	
	Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
		Calc_DistrQlearning_2Targs_BaselineDecay([alpha_A, decay_A, beta_A], 
	...
	x0_Ap = [alpha_A, decay_A, beta_A]
	
	result_Ap = op.minimize(nll, x0_Ap,
							args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
							bounds=[(0,1), (0.9,1), (0.1,50)],
							method='Nelder-Mead')
	alpha_Ap, decay_Ap, beta_Ap = result_Ap["x"]
	
	print(f"  alpha: {alpha_Ap:.4f}, decay: {decay_Ap:.4f}, beta: {beta_Ap:.4f}")
	print("="*60 + "\n")
	
	Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
		Calc_DistrQlearning_2Targs_BaselineDecay([alpha_Ap, decay_Ap, beta_Ap], 
	...
	# BACKWARDS COMPATIBILITY: Return alpha twice for each block, set unused params to None
	return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
			alpha_A, alpha_A, decay_A, None, None, beta_A,
			alpha_Ap, alpha_Ap, decay_Ap, None, None, beta_Ap)
