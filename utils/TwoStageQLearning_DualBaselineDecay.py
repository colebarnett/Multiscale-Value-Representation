# -*- coding: utf-8 -*-
"""
DUAL-ALPHA BASELINE + DECAY Q-Learning Model
Parameters: pos_alpha, neg_alpha, decay, beta (4 params)
NO stimulation, NO lapse, NO kernel, NO adaptive learning

Asymmetric learning (wins vs losses) + forgetting.
Tests if dual learning rates + decay are sufficient.
"""
...
def Calc_DistrQlearning_2Targs_DualBaselineDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	'''Dual-alpha Q-learning with decay'''
	pos_alpha = parameters[0]  # Learning rate for rewards
	neg_alpha = parameters[1]  # Learning rate for no rewards
	decay = parameters[2]      # Forgetting/decay parameter
	beta = parameters[3]       # Inverse temperature
	...
	for i in range(len(chosen_target)-1):
		# DECAY/FORGETTING: Q-values decay toward 0.5 (neutral value)
...
def loglikelihood_DistrQlearning_2Targs_DualBaselineDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice):
	Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total = \
		Calc_DistrQlearning_2Targs_DualBaselineDecay(parameters, Q_initial, chosen_target, rewards, instructed_or_freechoice)
	return log_prob_total


def DistrQlearning_2Targs_TwoStage_DualBaselineDecay(chosen_target, rewards, instructed_or_freechoice, num_trials_A, num_trials_B):
	"""
	Dual-alpha + Decay model - fit Block A and A' separately
	Only 4 parameters: pos_alpha, neg_alpha, decay, beta
	Asymmetric learning with forgetting
	"""
	...
	print("\n" + "="*60)
	print("MODEL: Dual-Alpha Baseline + Decay")
	print("Asymmetric learning + forgetting")
	print("Parameters: pos_alpha, neg_alpha, decay, beta ONLY")
	print("STAGE 1: Fitting Block A")
	print("="*60)
	
	Q_initial = 0.5 * np.ones(2)
	nll = lambda *args: -loglikelihood_DistrQlearning_2Targs_DualBaselineDecay(*args)
	x0 = [0.5, 0.5, 0.95, 3.0]  # [pos_alpha, neg_alpha, decay, beta]
	
	result_A = op.minimize(nll, x0, 
						   args=(Q_initial, chosen_target_A, rewards_A, instructed_A),
						   bounds=[(0,1), (0,1), (0.9,1), (0.1,50)],
						   method='Nelder-Mead')
	pos_alpha_A, neg_alpha_A, decay_A, beta_A = result_A["x"]
	
	print(f"  pos_alpha: {pos_alpha_A:.4f}, neg_alpha: {neg_alpha_A:.4f}")
	print(f"  decay: {decay_A:.4f}, beta: {beta_A:.4f}")
	
	Q_low_A, Q_high_A, prob_choice_low_A, prob_choice_high_A, accuracy_A, log_prob_A = \
		Calc_DistrQlearning_2Targs_DualBaselineDecay([pos_alpha_A, neg_alpha_A, decay_A, beta_A], 
	...
	x0_Ap = [pos_alpha_A, neg_alpha_A, decay_A, beta_A]
	
	result_Ap = op.minimize(nll, x0_Ap,
							args=(Q_initial_Ap, chosen_target_Ap, rewards_Ap, instructed_Ap),
							bounds=[(0,1), (0,1), (0.9,1), (0.1,50)],
							method='Nelder-Mead')
	pos_alpha_Ap, neg_alpha_Ap, decay_Ap, beta_Ap = result_Ap["x"]
	
	print(f"  pos_alpha: {pos_alpha_Ap:.4f}, neg_alpha: {neg_alpha_Ap:.4f}")
	print(f"  decay: {decay_Ap:.4f}, beta: {beta_Ap:.4f}")
	print("="*60 + "\n")
	
	Q_low_Ap, Q_high_Ap, prob_choice_low_Ap, prob_choice_high_Ap, accuracy_Ap, log_prob_Ap = \
		Calc_DistrQlearning_2Targs_DualBaselineDecay([pos_alpha_Ap, neg_alpha_Ap, decay_Ap, beta_Ap], 
	...
	return (Q_low, Q_high, prob_choice_low, prob_choice_high, accuracy, log_prob_total,
			pos_alpha_A, neg_alpha_A, decay_A, None, None, beta_A,
			pos_alpha_Ap, neg_alpha_Ap, decay_Ap, None, None, beta_Ap)
